...
export class HojaVidaPacientesService {
  ...
  async getAll(): Promise<HojaVida[]> {
    try {
      ...
      throw new Error('Error retrieving all Hoja de Vida Pacientes');
    }
  }

  // Obtener una hoja de vida por ID
  async getHojaVidaPaciente(id: number): Promise<HojaVida | null> {
    try {
      const [rows] = await connection.query<RowDataPacket[]>(
        'SELECT * FROM HOJAS_VIDA WHERE idHoja_Vida = ?',
        [id]
      );
      // Si no se encuentra una hoja de vida, devolver null
      return rows.length > 0 ? (rows[0] as HojaVida) : null;
    } catch (error) {
      console.error('Error en getHojaVidaPaciente:', error);
      throw new Error('Error retrieving Hoja de Vida Paciente by ID');
    }
  }

  // Crear una nueva hoja de vida
  async createHojaVidaPaciente(data: HojaVida): Promise<ResultSetHeader> {  // Cambiado el nombre del método
    try {
      ...
      console.error('Error en createHojaVidaPaciente:', error);
      throw new Error('Error creating Hoja de Vida Paciente');
    }
  }

  // Actualizar una hoja de vida
  async updateHojaVidaPaciente(id: number, data: HojaVida): Promise<ResultSetHeader> {
    try {
      const [result] = await connection.query<ResultSetHeader>(
        `UPDATE HOJAS_VIDA SET 
        direccion = ?, estadoUsuario = ?, telefonoUsuario = ?, idEps = ?, tipo_documento = ?, 
        sexo = ?, nacionalidad = ?, pais = ?, fecha_nacimiento = ?, lugar_nacimiento = ?, 
      ...
      console.error('Error en updateHojaVidaPaciente:', error);
      throw new Error('Error updating Hoja de Vida Paciente');
    }
  }

  // Eliminar una hoja de vida por ID
  async deleteHojaVidaPaciente(id: number): Promise<ResultSetHeader> {
    try {
      const [result] = await connection.query<ResultSetHeader>(
      ...
      console.error('Error en deleteHojaVidaPaciente:', error);
