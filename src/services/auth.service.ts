...
export function generarToken(userId: string) {
    ...
    const secret = process.env.TOKEN_SECRET || 'tu_clave_secreta'; // Asegúrate de tener esto en tu archivo .env
...
export function generarTokenSede(sede: SedeLogIn, ip: string): string {
    ...
    const secret = process.env.TOKEN_SECRET || 'vitamed'; // Usa una clave segura
