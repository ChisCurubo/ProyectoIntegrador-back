...
export class HojaVidaEmpleadosController {
  ...
  handleAction = async (req: Request, res: Response) => {
    ...
    try {
      switch (action) {
        case 'getAll':
          const allRecords = await this.service.getAllEmployer();
          res.json(allRecords);
          break;
        ...
        case 'getById':
          ...
          const record = await this.service.getByIdEmployer(id);
          if (record) {
            res.json(record);
          } else {
        ...
        case 'create':
          const newRecord = await this.service.createEmployer(req.body);
        ...
        case 'update':
          ...
          await this.service.updateEmployer(id, req.body);
          res.status(204).send();
          break;
