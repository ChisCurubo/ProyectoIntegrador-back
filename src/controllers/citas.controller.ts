...
import { BadRequestError, InternalServerError, NotFoundError } from "../middlewares/customErrors";
...
export class CitasController {
  ...
  public async createCita(req: Request, res: Response, next: NextFunction): Promise<void> {
    const { fecha, hora, medico, idUsuario, historialMedico, idServicio } = req.body;
    try {
      const result = await CitasService.createCita(fecha, hora, medico, idUsuario, historialMedico, idServicio);
  ...
  public async updateCita(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { idCita } = req.params; // ID de la cita
      const { nuevaFecha, nuevaHora } = req.body;
      console.log("id", idCita, nuevaHora, nuevaFecha);

      if (!nuevaFecha) {
        res.status(400).json({ error: "La nueva fecha y hora son requeridas" });
      }
      const response = await CitasService.updateCita(idCita, nuevaFecha, nuevaHora);
